...
def merge_eos_token_ids(eos_token_id: int | list[int] | None, additional_eos_token_ids: list[int]):
    """Return the original EOS value plus any runtime-only additional EOS IDs."""
    if not additional_eos_token_ids:
        return eos_token_id

    base_ids = eos_token_id if isinstance(eos_token_id, (list, tuple)) else [eos_token_id]
    merged_ids = []
    for token_id in [*base_ids, *additional_eos_token_ids]:
        if token_id is not None and token_id not in merged_ids:
            merged_ids.append(int(token_id))
    return merged_ids


def apply_additional_eos_token_ids(hf_config, generation_config, additional_eos_token_ids: list[int]):
    """Patch loaded config objects in memory without modifying their source files."""
    if not additional_eos_token_ids:
        return

    model_eos_token_id = getattr(hf_config, "eos_token_id", None)
    hf_config.eos_token_id = merge_eos_token_ids(model_eos_token_id, additional_eos_token_ids)
    if generation_config is not None:
        generation_eos_token_id = getattr(generation_config, "eos_token_id", None)
        if generation_eos_token_id is None:
            generation_eos_token_id = model_eos_token_id
        generation_config.eos_token_id = merge_eos_token_ids(
            generation_eos_token_id, additional_eos_token_ids
        )
...
class HFModelConfig(BaseConfig):
    ...
    additional_eos_token_ids: list[int] = field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        apply_additional_eos_token_ids(
            self.hf_config, self.generation_config, self.additional_eos_token_ids
        )
    ...
    @property
    def effective_eos_token_ids(self):
        eos_token_id = None
        if self.generation_config is not None:
            eos_token_id = self.generation_config.eos_token_id
        if eos_token_id is None and self.hf_config is not None:
            eos_token_id = self.hf_config.eos_token_id
        return merge_eos_token_ids(eos_token_id, self.additional_eos_token_ids)
