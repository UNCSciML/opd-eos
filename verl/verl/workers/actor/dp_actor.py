...
from verl.utils.eos_semantics import student_policy_log_probs, validate_eos_mode
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        tracked_token_ids=None,
        eos_mode="baseline",
        semantic_eos_token_ids=None,
        ...
        Returns:
            ...
            tracked_token_log_probs: # (bs, response_len, num_tracked_tokens)
        """
        eos_mode = validate_eos_mode(eos_mode)
        semantic_eos_token_ids = [int(token_id) for token_id in (semantic_eos_token_ids or [])]
        needs_eos_policy = eos_mode in {"semantic_class", "canonical"}
        if eos_mode == "semantic_class" and (
            not semantic_eos_token_ids or len(set(semantic_eos_token_ids)) != len(semantic_eos_token_ids)
        ):
            raise ValueError("semantic_class requires a non-empty list of distinct semantic EOS token ids")
        if eos_mode == "canonical" and (
            len(semantic_eos_token_ids) != 2 or semantic_eos_token_ids[0] == semantic_eos_token_ids[1]
        ):
            raise ValueError("canonical requires exactly two distinct semantic EOS token ids")
        if eos_mode == "canonical" and torch.any(micro_batch["responses"] == semantic_eos_token_ids[1]):
            raise ValueError(f"Canonical EOS policy sampled blocked EOS token {semantic_eos_token_ids[1]}")
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                need_tracked_tokens = tracked_token_ids is not None and len(tracked_token_ids) > 0
                need_logits = top_k > 0 or need_tracked_tokens or needs_eos_policy
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if need_topk:
                        ...
                        if tracked_token_ids is not None and len(tracked_token_ids) > 0:
                            tracked_token_log_probs = verl_F.gather_log_probs_for_token_ids(
                                log_probs_all, tracked_token_ids
                            )
                    else:
                        ...
                        if need_tracked_tokens:
                            tracked_token_log_probs = verl_F.gather_log_probs_for_token_ids_from_logits(
                                logits_rmpad, tracked_token_ids
                            )

                    if needs_eos_policy:
                        policy_labels = input_ids_rmpad_rolled
                        if eos_mode == "canonical":
                            e1, e2 = semantic_eos_token_ids
                            policy_labels = torch.where(policy_labels == e2, e1, policy_labels)
                        log_probs = student_policy_log_probs(
                            logits=logits_rmpad,
                            sampled_ids=policy_labels,
                            raw_sampled_log_probs=log_probs,
                            mode=eos_mode,
                ...
                if self.use_ulysses_sp:
                    ...
                    if tracked_token_log_probs is not None:
                        tracked_token_log_probs = gather_outputs_and_unpad(
                            tracked_token_log_probs,
                            gather_dim=0,
                            unpad_dim=0,
                            padding_size=pad_size,
                        )
                ...
                if tracked_token_log_probs is not None:
                    full_tracked_token_log_probs = pad_input(
                        hidden_states=tracked_token_log_probs,
                        indices=indices,
                        batch=batch_size,
                        seqlen=seqlen,
                    )
                ...
                if tracked_token_log_probs is not None:
                    tracked_token_log_probs = full_tracked_token_log_probs[:, -response_length - 1 : -1, :]
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if need_topk:
                        ...
                        if need_tracked_tokens:
                            tracked_token_log_probs = verl_F.gather_log_probs_for_token_ids_from_logits(
                                logits, tracked_token_ids
                            )

                    if needs_eos_policy:
                        log_probs = student_policy_log_probs(
                            logits=logits,
                            sampled_ids=micro_batch["responses"],
                            raw_sampled_log_probs=log_probs,
                            mode=eos_mode,
                            semantic_eos_token_ids=semantic_eos_token_ids,
                        )
            ...
            return entropy, log_probs, topk_ids, topk_log_probs, tracked_token_log_probs
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        use_dynamic_bsz = data.meta_info["use_dynamic_bsz"]
        tracked_token_ids = data.meta_info.get("tracked_token_ids", [])
        eos_mode = data.meta_info.get("eos_mode", "baseline")
        semantic_eos_token_ids = data.meta_info.get("semantic_eos_token_ids", [])
        has_multi_modal_inputs = "multi_modal_inputs" in data.non_tensor_batch.keys()
        select_keys = ["responses", "input_ids", "attention_mask", "position_ids"]
        non_tensor_select_keys = ["multi_modal_inputs"] if has_multi_modal_inputs else []
        ...
        topk_ids_lst = []
        topk_log_probs_lst = []
        tracked_token_log_probs_lst = []

        for micro_batch in micro_batches:
            micro_batch = micro_batch.to(get_device_id())
            model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
            with torch.no_grad():
                entropy, log_probs, topk_ids, topk_log_probs, tracked_token_log_probs = self._forward_micro_batch(
                    model_inputs,
            ...
            if top_k > 0:
                ...
                topk_log_probs_lst.append(topk_log_probs)
            if tracked_token_log_probs is not None:
                tracked_token_log_probs_lst.append(tracked_token_log_probs)
        ...
        tracked_token_log_probs_tensor = None
        if tracked_token_log_probs_lst:
            tracked_token_log_probs_tensor = torch.concat(tracked_token_log_probs_lst, dim=0)
        ...
        if use_dynamic_bsz:
            ...
            if top_k > 0:
                ...
                topk_log_probs_tensor = restore_dynamic_batch(topk_log_probs_tensor, batch_idx_list)
            if tracked_token_log_probs_tensor is not None:
                tracked_token_log_probs_tensor = restore_dynamic_batch(
                    tracked_token_log_probs_tensor, batch_idx_list
                )

        return log_probs, entropys, topk_ids_tensor, topk_log_probs_tensor, tracked_token_log_probs_tensor
                ...
                for micro_batch in micro_batches:
                    ...
                    if advantages.dim() == 3:
                        ...
                        entropy, _, _, topk_log_probs, _ = self._forward_micro_batch(
                            ...
                            eos_mode=eos_mode, semantic_eos_token_ids=semantic_eos_token_ids,
                        )
