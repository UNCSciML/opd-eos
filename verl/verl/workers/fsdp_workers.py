...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        from verl.workers.config.model import apply_additional_eos_token_ids
        ...
        apply_additional_eos_token_ids(
            actor_model_config,
            self.generation_config,
            self.config.model.get("additional_eos_token_ids", []),
        )
        if self.rank == 0:
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        data.meta_info["tracked_token_ids"] = list(self.config.rollout.get("tracked_token_ids", []))
        data.meta_info["eos_mode"] = self.config.rollout.get("eos_mode", "baseline")
        data.meta_info["semantic_eos_token_ids"] = list(
            self.config.rollout.get("semantic_eos_token_ids", [])
        )
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, topk_ids, topk_log_probs, tracked_token_log_probs = self.actor.compute_log_prob(
            ...
            if tracked_token_log_probs is not None:
                tensors["student_tracked_token_log_probs"] = tracked_token_log_probs
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(
        ...
        if eos_mode in {"teacher_map", "canonical"} and (
            len(semantic_eos_token_ids) != 2 or semantic_eos_token_ids[0] == semantic_eos_token_ids[1]
        ):
            raise ValueError(f"{eos_mode} requires exactly two distinct semantic EOS token ids")
        if eos_mode in {"two_stop", "semantic_class"} and (
            not semantic_eos_token_ids or len(set(semantic_eos_token_ids)) != len(semantic_eos_token_ids)
        ):
            raise ValueError(f"{eos_mode} requires a non-empty list of distinct semantic EOS token ids")
        track_teacher_eos = bool(semantic_eos_token_ids)
        ...
        with torch.no_grad(), torch.autocast(device_type=get_device_name(), dtype=torch.bfloat16):
            ...
            need_logits = student_top_k_ids is not None or compute_entropy or track_teacher_eos
            ...
            if self.use_remove_padding:
                ...
                local_teacher_eos_log_probs = None
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if track_teacher_eos:
                        local_teacher_eos_log_probs = gather_fixed_token_log_probs(
                            logits_rmpad, semantic_eos_token_ids
                        )
                ...
                if self.use_ulysses_sp:
                    ...
                    if local_teacher_eos_log_probs is not None:
                        full_teacher_eos_log_probs = gather_outputs_and_unpad(
                            local_hidden_states=local_teacher_eos_log_probs,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                            sp_size=self.ulysses_sequence_parallel_size,
                    ...
                    if local_teacher_eos_log_probs is not None:
                        full_teacher_eos_log_probs = pad_input(
                            hidden_states=local_teacher_eos_log_probs,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                        )
                ...
                if local_teacher_eos_log_probs is not None:
                    teacher_eos_log_probs = full_teacher_eos_log_probs[:, -response_length - 1 : -1, :]
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if track_teacher_eos:
                        teacher_eos_log_probs = gather_fixed_token_log_probs(
                            rm_logits_resp, semantic_eos_token_ids
                        )
            ...
            if eos_mode in {"teacher_map", "semantic_class", "canonical"}:
                rm_log_probs = map_teacher_sampled_log_probs(
                    raw_sampled_log_probs=rm_log_probs,
                    teacher_eos_log_probs=teacher_eos_log_probs,
                    sampled_ids=micro_batch["responses"],
                    mode=eos_mode,
                    semantic_eos_token_ids=semantic_eos_token_ids,
                    epsilon=eos_mapping_epsilon,
                )
    ...
    def compute_rm_score(self, data: DataProto, kl_estimator="k1"):
        ...
        with self.ulysses_sharding_manager:
            ...
            semantic_eos_token_ids = list(data.meta_info.get("semantic_eos_token_ids", []))
            eos_mapping_epsilon = float(data.meta_info.get("eos_mapping_epsilon", 1e-12))
