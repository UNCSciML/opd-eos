...
from verl.trainer.ppo.metric_utils import (
    attach_eos_semantics_metadata,
    compute_data_metrics,
    compute_eos_diagnostic_metrics,
    compute_throughout_metrics,
    compute_timing_metrics,
    compute_tracked_token_probability_metrics,
    extract_teacher_eos_at_student_eos,
    process_validation_metrics,
    save_teacher_eos_diagnostic_npz,
)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        eos_mode, semantic_eos_token_ids, eos_mapping_epsilon = attach_eos_semantics_metadata(
                            batch, self.config.actor_rollout_ref.rollout
                        )
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if "student_tracked_token_log_probs" in batch.batch:
                            student_eos_log_probs = batch.batch["student_tracked_token_log_probs"]
                            tracked_token_ids = list(
                                self.config.actor_rollout_ref.rollout.get("tracked_token_ids", [])
                            )
                            tracked_token_names = list(
                                self.config.actor_rollout_ref.rollout.get(
                                    "tracked_token_names",
                                    [f"token_{token_id}" for token_id in tracked_token_ids],
                                )
                            )
                            if "teacher_eos_log_probs" in batch.batch and semantic_eos_token_ids:
                                if tracked_token_ids != semantic_eos_token_ids:
                                    raise ValueError(
                                        "tracked_token_ids must match semantic_eos_token_ids in the same order"
                                    )
                                eos_diagnostic_output_dir = self.config.trainer.get(
                                    "eos_diagnostic_output_dir", None
                                )
                                if eos_diagnostic_output_dir:
                                    eos_records = extract_teacher_eos_at_student_eos(
                                        responses=batch.batch["responses"],
                                        response_mask=batch.batch["response_mask"],
                                        teacher_eos_log_probs=batch.batch["teacher_eos_log_probs"],
                                        terminal_token_ids=semantic_eos_token_ids,
                                    )
                                    eos_output_path = save_teacher_eos_diagnostic_npz(
                                        output_dir=eos_diagnostic_output_dir,
                                        global_step=self.global_steps,
                                        eos_token_ids=semantic_eos_token_ids,
                                        records=eos_records,
                                    )
                                    eos_count = len(eos_records["response_length"])

                                    print(
                                        "[teacher-eos-at-student-eos] "
                                        + json.dumps(
                                            {
                                                "step": self.global_steps,
                                                "count": eos_count,
                                                "eos_token_ids": semantic_eos_token_ids,
                                                "teacher_eos_mass_mean": (
                                                    float(eos_records["teacher_eos_mass"].mean())
                                                    if eos_count else None
                                                ),
                                                "output": str(eos_output_path),
                                            },
                                            sort_keys=True,
                                        )
                                    )
                                metrics.update(
                                    compute_eos_diagnostic_metrics(
                                        ...
                                        student_eos_log_probs=student_eos_log_probs,
                                        teacher_eos_log_probs=batch.batch["teacher_eos_log_probs"],
                                        eos_token_ids=semantic_eos_token_ids,
                                        token_names=tracked_token_names,
                                    )
                                )
                            else:
                                metrics.update(
                                    compute_tracked_token_probability_metrics(
                                        tracked_token_log_probs=student_eos_log_probs,
                                        response_mask=batch.batch["response_mask"],
                                        token_names=tracked_token_names,
                                    )
                                )
                            batch.batch.pop("student_tracked_token_log_probs")
                        if "teacher_eos_log_probs" in batch.batch:
                            batch.batch.pop("teacher_eos_log_probs")
                    ...
                    keys_to_pop = [
                        ...
                        "student_tracked_token_log_probs",
