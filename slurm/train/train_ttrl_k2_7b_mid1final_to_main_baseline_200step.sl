#!/usr/bin/env bash
#SBATCH --job-name=opd_k2_7b_mid1final_base200
...
#SBATCH --gres=gpu:4
# K2-Horizon-7B student = `mid_1_final` (already EOT-preferring stage) <- teacher = main, vanilla OPD (EOS_MODE=baseline).
...
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-$K2_MODELS_ROOT/K2-Horizon-7B-mid_1_final-llamaview}
export RAY_MIN_WORKER_PORT=${RAY_MIN_WORKER_PORT:-40000} RAY_MAX_WORKER_PORT=${RAY_MAX_WORKER_PORT:-44999}
export TOTAL_TRAINING_STEPS=${TOTAL_TRAINING_STEPS:-200}
RUN_SUFFIX=${SLURM_JOB_ID:-manual}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-opd_ttrl_k2_7b_mid1final_to_main_baseline_${RUN_SUFFIX}}
