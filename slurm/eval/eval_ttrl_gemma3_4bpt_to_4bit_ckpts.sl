#!/usr/bin/env bash
#SBATCH --job-name=opd_g3_4b_eval
...
# about 80 GB under checkpoint_merged/<run>/ for Gemma-3-4B. The generations
...
#
# Checkpoint eval for any Gemma-3-4B-PT -> Gemma-3-4B-IT TTRL run, i.e. either
# of:
#   slurm/train/train_ttrl_gemma3_4bpt_to_4bit_baseline_200step.sl
#   slurm/train/train_ttrl_gemma3_4bpt_to_4bit_semantic_class_200step.sl
#
# This wrapper adds no eval semantics of its own. It only supplies the Slurm
# resources a 4B student needs and then runs the same checkpoint evaluator used
...
#   wandb login
#   export CHECKPOINT_ROOT="$PWD/checkpoint/opd_ttrl_gemma3_4bpt_to_4bit_baseline_<train-job-id>"   # or the _semantic_class_ run
...
#        --gres=gpu:4 --cpus-per-task=16 --mem=200G --time=3-00:00:00 \
#        --export=ALL slurm/eval/eval_ttrl_gemma3_4bpt_to_4bit_ckpts.sl
...
# Merged HF checkpoints are written to checkpoint_merged/<run>/; ten merged 4B
# models need roughly 90 GB of scratch space. Set MERGED_ROOT to relocate them.
...
if [ -z "$CHECKPOINT_ROOT" ]; then
    ...
    echo "  CHECKPOINT_ROOT=\"\$PWD/checkpoint/opd_ttrl_gemma3_4bpt_to_4bit_baseline_<train-job-id>\" \\" >&2
    echo "    sbatch --export=ALL slurm/eval/eval_ttrl_gemma3_4bpt_to_4bit_ckpts.sl" >&2
    exit 2
fi
